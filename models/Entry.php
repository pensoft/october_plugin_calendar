...
class Entry extends Model
{
    ...
    {
        if (isset($fields->end) && isset($fields->start)) {
            $startValue = $fields->start->value;
            if ($startValue) {
                $fields->end->minDate = $startValue;

                // Set end default to start value if end is empty or before start
                if (empty($fields->end->value) || $fields->end->value < $startValue) {
                    $fields->end->value = $startValue;
                }
            }
        }
    }
